...
export const fetchSessoes = async (): Promise<Sessao[]> => {
  const response = await api.get('sessoes?_expand=filme&_expand=sala')
  return response.data
}

export const createSessao = async (sessao: Omit<Sessao, 'id'>): Promise<Sessao> => {
  const response = await api.post('sessoes', sessao)
  return response.data
}

export const deleteSessao = async (id: number): Promise<void> => {
...
export const createIngresso = async (ingresso: Omit<Ingresso, 'id'>): Promise<Ingresso> => {
  const response = await api.post('ingressos', ingresso)
  return response.data
}

// Adicione esta função à api.ts
export const updateSala = async (id: number, sala: Sala): Promise<Sala> => {
